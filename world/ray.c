...
/*   Updated: 2019/01/22 06:19:19 by izoukhai    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static void ray_dist(env_t *env)
{
    ...
    {
        env->step.x = -1;
        env->side_dist.x = (env->player.raypos.x - env->map_x) * env->delta_dist.x;
    }
    else
    {
        env->step.x = 1;
        env->side_dist.x = (env->map_x + 1.0 - env->player.raypos.x) * env->delta_dist.x;
    }
    if (env->player.raydir.y < 0)
    {
        env->step.y = -1;
        env->side_dist.y = (env->player.raypos.y - env->map_y) * env->delta_dist.y;
    }
    else
    {
        env->step.y = 1;
        env->side_dist.y = (env->map_y + 1.0 - env->player.raypos.y) * env->delta_dist.y;
    }
}

static void ray_test(env_t *env)
{
    env->ray_hit = 0;
    while (env->ray_hit == 0)
    {
...
void        init_ray(env_t *env, int x)
{
    ...
    if (env->type == 0)
        env->wall_dist = (env->map_x - env->player.raypos.x +
            (1 - env->step.x) / 2) / env->player.raydir.x;
    else
...
void        ray_cast(env_t *env)
{
    ...
    while (++x < WIN_W)
    {
        ...
        if (env->end >= WIN_H)
            env->end = WIN_H - 1;
        if (env->type == 1)
            env->color = RGB(140, 70, 170);
        else
            env->color = (RGB(170, 70, 210));
        draw_wall(env, x);
        //draw_floor(env);
    }
